...
class Game(GTMS.ITF, GTTS.ITF):
    ...
    def calc_score_entry(self, entry: LmScore) -> float:
        """
        :param entry: LmScore object
        :return: Floating value (the teams calculated score based on time spent for that landmark)
        """
        output : float = 0
        if entry.correct:
    ...
    def req_team_progress(self, team: User):
        """
        :param team: User object (model)
        :return: output list with the teams progress (current landmark, score, etc.)
        """
    ...
    def edit_lmark(self, lm: Landmark, name: str, desc: str) -> bool:
        """
        :param lm: Landmark object (check if exists in DB && in specific game)
        :param name: name type string (for new name of landmark)
        :param desc: desc type string (for new desc of landmark)
        :return: boolean value if landmark was successfully edited
        """
    ...
    def reorder_hunt(self, order: [int]) -> bool:
        """
        :param order: integer array (new order)
        :return: boolean value (true if reordering successful)
        """
    ...
    def edit_clue(self, lm: Landmark, value: str) -> bool:
        """
        :param lm: Landmark object (used to identify which LM clue we are editing)
        :param value: value type string (edited clue)
        :return: boolean value (true if clue was edited successfully)
        """
        if type(value) != str:
    ...
    def edit_conf(self, lm: Landmark, ques: str, ans: str) -> bool:
        """
        :param lm: Landmark object (of which we would like to edit it's confirmation question)
        :param ques: question type string (the new question for our landmark)
        :param ans: answer type string (the new answer to our new question)
        :return: boolean return value (returns true if confirmation question successfully edited)
        """
        if type(ques) != str:
    ...
    def mk_team(self, name: str, pwd: str) -> bool:
        """
        :param name: name type string (username used to create user - same username cannot exist)
        :param pwd: password type string (password used to create user)
        :return: boolean return value (returns true if team successfully created)
        """
    ...
    def rm_lmark(self, index: int) -> bool:
        """
        :param index: index type integer  (the index h_order of the landmark you would like to remove)
        :return: boolean return value (returns true if landmark successfully removed)
        """
    ...
    def rm_team(self, team: User) -> bool:
        """
        :param team: team type User object (searches for team in game - if exists deletes)
        :return: boolean return value (returns true if team successfully removed)
        """
        try:
    ...
    def edit_team_creds(self, team: User, name: str, pwd: str) -> bool:
        """
        :param team: team type User object (used to identify which team we are editing)
        :param name: name type string (new username for specified user)
        :param pwd: password type string (new password for specified user)
        :return: boolean return value (true if user's/team's credentials edited correctly)
        """
        try:
    ...
    def mk_score_sch(self, name: str) -> bool:
        """
        :param name: name type string (creating scoreScheme object with specified name)
        :return: boolean return value (returns true if no other scoreScheme exists with that name && is type str)
        """
        if type(name) != str:
    ...
    def edit_score_sch(self, scheme: ScoreScheme) -> bool:
        """
        :param scheme: scheme type ScoreScheme object (fields of this object replace current ScoreScheme)
        :return: boolean return value (returns true if scoreScheme successfully edited)
        """
    ...
    def req_status(self, team : User) -> {GameDetails, str, bool, float}:
        """
        :param team: team type User object (used to find which user you're returning the status of)
        :return: object (fields contain teams status)
        """
        try:
            u = User.objects.get(pk = team.pk)
        except:
    ...
    def req_hunt(self) -> [{str, int}]:
        """
        :return: array of objects (fields name and order returning the landmark names and their order in the hunt)
        """
        output : [{str, int}] = []
        for h in self.hunt:
    ...
    def req_team_statuses(self) -> [{User, Status, int}]:
        """
        :return: returns array of objects (fileds object User, object Status and curr index for every team)
        """
    ...
    def edit(self, name: str, desc: str) -> bool:
        """
        :param name: name type string (name of game changed - game details)
        :param desc: desc type string (desc of game changed - game details)
        :return: boolean return value (true when edited successfully - i.e. no error raised)
        """
    ...
    def start(self) -> bool:
        """
        :return: boolean return value (true when started successfully - i.e. no userwarning)
        """
        if self.is_on():
    ...
    def stop(self) -> bool:
        """
        :return: boolean return value (True when game stopped successfully - i.e. no UserWarning raised)
        """
        if not self.is_on():
    ...
    def is_on(self) -> bool:
        """
        :return: boolean return value (True when self.dtls.on == true - i.e. the game is on)
        """
        return self.dtls.on
    ...
    def set_winner_by_score(self, team: User) -> User:
        """
        :param team: User object (method checks that User exists in game)
        :return: string return value of winning teams name
        """
        try:
            u = User.objects.get(pk = team.pk)
            if Status.objects.filter(team = team, game = self.dtls).count() == 0:
    ...
    def set_winner(self, team : User) -> bool:
        """
        :param team: team User object (winner of game (set by set_winner_by_score))
        :return: boolean return value (returns true if game off - i.e. it's time for a winner to be set)
        """
    ...
    def req_clue(self, team: User) -> Clue:
        """
        :param team: team User object (checks numerous things before giving this team a clue)
        :return: Clue type string return value (from Clue object)
        """
        if not self.is_on():
    ...
    def req_ques(self, team: User) -> str:
        """
        :param team: team User object (checks numerous things before giving this team a question)
        :return: question type string return value (from Confirmation object)
        """
        if not self.is_on():
            raise IndexError("Game is not on")
        s = Status.objects.filter(team = team, game = self.dtls)
    ...
    def submit_ans(self, team: User, ans: str) -> bool:
        """
        :param team: team User object (gives answer submission for a certain landmark for this team)
        :param ans: answer type string (answer given by team)
        :return: boolean return value (true if question answered correctly - current landmark index incremented)
        """
        if not self.is_on():
            raise IndexError("Game is not on")
        s = Status.objects.filter(team = team, game = self.dtls)
    ...
    def forfeit(self, team: User) -> bool:
        """
        :param team: team User object (checks numerous things before forfeiting this team - e.g. already forfeited)
        :return: boolean return value (true if successfully forfeited - i.e. no errors thrown)
        """
        try:
