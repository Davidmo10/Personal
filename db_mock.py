...
from escavenger_db_exceptions import InvalidGameException,  InvalidTeamException,  InvalidUserException, InvalidLandmarkException,  InvalidGameStateException, InvalidTeamStateException, EmptyGameSetException, EmptyTeamListException, LandmarkAlreadyInGameException, SettingAnswerWithNoQuestionException
import copy
...
class DBMock:
	...
	@staticmethod
	def fetch_team(game_id, team_id):
...
class Team_DBMock(TeamDBInterface, DBMock):
	...
	def forfeit(self, game_id, team_id):
		self.g = DBMock.fetch_game(game_id)
		self.t = DBMock.fetch_team(game_id, team_id)
		if self.g["on"] == 0:
...
class Master_DBMock(MasterDBInterface, DBMock):
	...
	def create_game(self, master_id, title = "Default Title", desc = "Default Description"):
		...
		self.games.append({"game_id" : new_id , "title" : title, "desc" : desc, "master_id" : master_id, "on" : 0, "landmarks" : (), "teams" : () })
		return new_id
...
class Game_DBMock(GameDBInterface, DBMock):
...
class Landmark_DBMock(LandmarkDBInterface, DBMock):
	...
	def set_question(self, lm_id, game_id, prompt, answer = None):
		...
		if answer is not None:
			if flag == 0 and (l["question"] is None):
