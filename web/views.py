...
def login(request):
    """
    :param request: login request for backend. Exposes method that checks if uname and upwd are in database.
    :return: Rendered page with updated status
    """
...
def dash(request):
    """
    :param request: Holds information of what is trying to make the dash request
    :return: Rendered updated page with new status
    """
...
def do(request, type):
    """
    :param request: the type of action to be requested
    :param type: the type that the request will be performed on
    :return: rendered page with updated status
    """
    if request.session.get('loggedin', False):
        try:
...
def edit(request, type):
    """
    :param request: Request for the edit to be made
    :param type: The type that is edited by the request
    :return: Rendered page with new status
    """
    if request.session.get('loggedin', False):
        try:
            u = User.objects.get(name = request.session['name'])
